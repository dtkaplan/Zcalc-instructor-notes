...
subtitle: "*MOSAIC Calculus* Project"
---
...
For the previous answers you should use your calculated $\alpha$ and $\beta$ from before. In order to better understand the affect of these parameters, you can see what happens when the parameter changes aren't slight, try doubling $\beta$ while leaving $\alpha$ alone. You can accomplish this by inserting a line `beta <- ` your number in place of the existing calculation of $\beta$.  Use R to calculate what the doubled value of $\beta$ should be.
